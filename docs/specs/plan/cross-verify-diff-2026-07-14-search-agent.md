...
| 5 | MEDIUM | SyncEmbeddingsUseCase 미연결 | 오탐 | PR 이전부터 미연결, 이 PR 범위 아님 |
| 6 | LOW | API 키 mid-session 미반영 | 오탐 | PR 이전부터의 설계, 변경 없음 |
| 7 | LOW | 벡터 저장소 메타데이터 누락 | 오탐 | 기존 구현 이슈 |
...
## 측정
- Codex 단독 유효 지적: 4건 (HIGH 2, MEDIUM 2)
- 오탐: 3건 (pre-existing issues)
- 수용/후속: 2건
- 오탐률: 33% (3/9)
