...
### TC-2.2: 태그 제안 품질 (하이브리드 전략) + sanitize

| 항목 | 내용 |
|------|------|
| 확인 | 기존 태그와 강하게 관련된 경우 기존 태그를 재사용하는지 |
| 확인 | 기존 태그와 약하게만 매칭되는 경우 새 태그를 자유롭게 생성하는지 |
| 확인 | 새로 생성된 태그가 기존 태그와 의미적으로 겹치지 않는지 |
...
### TC-2.4: 링크 제안 (AI 기반 + vault 존재 검증)

| 항목 | 내용 |
|------|------|
| 확인 | 제안된 링크가 실제 존재하는 노트인지 (vault 존재 검증) |
| 확인 | AI가 hallucination으로 존재하지 않는 노트명을 제안해도 필터링되는지 |
| 확인 | 링크 경로에 .md 확장자가 표시되지 않는지 ([[Note Name]] 형식) |
| 확인 | 노트 내용과 의미적으로 관련 있는 노트가 제안되는지 (단순 문자열 매칭이 아닌 AI 기반) |
...
### TC-4.1: Command Palette에서 폴더 선택 + 사이드 패널 결과
...
| 확인 | ✅ 폴더 선택 시 **사이드 패널**이 열리고 프로그레스 바 + 카운터 + 현재 노트명 표시 |
| 확인 | ✅ 완료 시 노트별 상세 결과가 사이드 패널에 표시되는지 |
| 확인 | ✅ 각 노트에 카테고리 뱃지, 요약, 태그 칩, 링크 칩, 폴더 제안이 표시되는지 |
| 확인 | ✅ 토큰 사용량이 분류 + 링크 제안 합산으로 표시되는지 |

### TC-4.1b: 사이드 패널 Apply/Skip/Undo (autoApply=off)

| 항목 | 내용 |
|------|------|
| 설정 | Auto Apply = false |
| 준비 | 3개 이상의 노트 결과가 있는 상태 |
| 확인 | ✅ 각 노트 항목에 체크박스가 표시되는지 |
| 확인 | ✅ "Select All" 체크박스가 동작하는지 |
| 확인 | ✅ 개별 노트에서 "Apply" 클릭 시 태그/링크/이동이 적용되고 항목이 dim 처리되는지 |
| 확인 | ✅ 적용 후 "Undo" 버튼이 나타나는지 |
| 확인 | ✅ Undo 클릭 시 변경이 되돌려지고, 이동된 파일이 있으면 삭제되는지 |
| 확인 | ✅ "Apply Selected" 버튼으로 체크된 항목 일괄 적용이 동작하는지 |
| 확인 | ✅ "Skip Selected" 버튼으로 체크된 항목 건너뛰기가 동작하는지 |
| 확인 | ✅ 일괄 적용 시 실패한 노트 수가 정확히 보고되는지 |

### TC-4.1c: 사이드 패널 결과 확인 (autoApply=on)

| 항목 | 내용 |
|------|------|
| 설정 | Auto Apply = true |
| 확인 | ✅ 결과가 읽기 전용으로 표시되는지 (체크박스 없음) |
| 확인 | ✅ 각 항목에 Undo 버튼이 있는지 |
| 확인 | ✅ 태그/이동이 자동 적용된 상태인지 |

### TC-4.1d: 노트 열기 버튼 (이동 후)

| 항목 | 내용 |
|------|------|
| 준비 | 폴더 이동이 적용된 노트 |
| 확인 | ✅ "Open" 버튼 클릭 시 이동된 새 경로에서 노트가 열리는지 (원래 경로가 아닌) |
...
| 기대 | 폴더 선택 없이 바로 **사이드 패널** 열림 (선택한 폴더 자동 적용) |
| 확인 | ✅ 해당 폴더의 노트만 처리되는지 |
...
| 실행 | "Organize Folder" → 처리 중 Cancel 버튼 클릭 |
| 기대 | 현재 노트 처리 완료 후 중단, 요약에 처리된/미처리 카운트 표시 |
| 확인 | ✅ 취소 전까지 처리된 노트 수가 정확한지 |
...
### TC-4.8b: Undo로 이동 복원 시 중복 방지

| 항목 | 내용 |
|------|------|
| 준비 | 폴더 이동이 적용된 노트 (사이드 패널 또는 Activity Log에서 확인) |
| 실행 | Undo 버튼 클릭 (사이드 패널 또는 Activity Log) |
| 기대 | 원래 경로에 내용이 복원되고, 이동된 경로의 파일이 삭제됨 |
| 확인 | ✅ 원래 경로에 노트가 존재하는지 |
| 확인 | ✅ 이동된 경로에 노트가 남아있지 않은지 (중복 없음) |

### TC-4.8c: 태그 하이브리드 전략 (Organize Folder)

| 항목 | 내용 |
|------|------|
| 준비 | vault에 기존 태그가 10개 이상 있는 상태, 폴더에 다양한 주제 노트 배치 |
| 실행 | "Organize Folder" |
| 확인 | ✅ 기존 태그와 강하게 관련된 노트에는 기존 태그가 제안되는지 |
| 확인 | ✅ 기존 태그와 약하게만 관련된 노트에는 새 태그가 생성되는지 |
| 확인 | ✅ 새 태그가 기존 태그와 의미적으로 중복되지 않는지 |

### TC-4.8d: 링크 제안 AI 기반 + hallucination 방지 (Organize Folder)

| 항목 | 내용 |
|------|------|
| 준비 | 관련 노트가 있는 폴더 |
| 실행 | "Organize Folder" |
| 확인 | ✅ 제안된 링크가 모두 vault에 실제 존재하는 노트인지 |
| 확인 | ✅ 단순 문자열 매칭이 아닌 의미적으로 관련된 노트가 제안되는지 |
| 확인 | ✅ AI가 존재하지 않는 노트명을 제안해도 결과에 포함되지 않는지 |
...
| TC-4.1 | PASS / FAIL | Command Palette → 사이드 패널 결과 |
| TC-4.1b | PASS / FAIL | Apply/Skip/Undo (autoApply=off) |
| TC-4.1c | PASS / FAIL | 결과 확인 (autoApply=on) |
| TC-4.1d | PASS / FAIL | Open 버튼 (이동 후) |
...
| TC-4.8b | PASS / FAIL | Undo 이동 복원 중복 방지 |
| TC-4.8c | PASS / FAIL | 태그 하이브리드 전략 |
| TC-4.8d | PASS / FAIL | 링크 AI 기반 + hallucination 방지 |
